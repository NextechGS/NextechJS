...
- Have [commit access](https://github.com/NextechGS/NextechJS/blob/master/Documentation/Contributors/CommittersGuide/README.md) to NextechJS?
  - No
    - Fork [NextechJS](https://github.com/NextechGS/NextechJS).
    - Use the [GitHub website](https://github.com/NextechGS/NextechJS/branches/all) to delete all branches in your fork except `master`.
    - Clone your fork, e.g., `git clone git@github.com:yourusername/NextechJS.git`.
    - Make changes in a branch, e.g., `git checkout -b my-feature`.
  - Yes
    - Clone the NextechJS repo, e.g., `git clone git@github.com:NextechGS/NextechJS.git`
    - Make changes in a branch, e.g., `git checkout -b my-feature`.
...
NextechJS ships with a simple HTTP server for testing, run `npm start` after building to use it:
...
- **Test Suites** : tests using [Jasmine](https://jasmine.github.io/). [Testing guide here.](https://github.com/NextechGS/NextechJS/blob/master/Documentation/Contributors/TestingGuide/README.md#testing-guide)
- **Documentation** : reference documentation built from source. [Documentation guide here.](https://github.com/NextechGS/NextechJS/blob/master/Documentation/Contributors/DocumentationGuide/README.md#documentation-guide)

NextechJS can be used in two different ways. NextechJS can be either a set of modules using [Asynchronous Module Definition (AMD)](https://github.com/amdjs/amdjs-api/wiki/AMD), or it can be built as one combined file containing all modules. The basics:

- `npm run build` will build AMD NextechJS. This also builds Nextech Viewer and Sandcastle.
- `npm run minifyRelease` creates the built version of NextechJS. This also builds Hello World.
...
While you can use the editor of your choice to develop NextechJS, certain files, such as `glsl` and new tests, require that
the `build` task be executed in order for the changes to take effect. You can use the `build-watch` script to have this
happen automatically.

## Build Scripts

NextechJS uses [gulp](http://gulpjs.com/) for build tasks, but they are all abstracted away by [npm run scripts](https://docs.npmjs.com/cli/run-script).
...
- **Build scripts** -- build and package the source code and documentation
  ...
  - `build-watch` - A never-ending task that watches your file system for changes to NextechJS and runs `build` on the source code as needed.
  - `combine` - Runs `build`, plus the [the RequireJS optimizer](http://requirejs.org/docs/optimization.html) to combine NextechJS and [the Almond AMD loader](http://requirejs.org/docs/faq-optimization.html#wrap) to produce all-in-one files in the `Build/Nextech` directory that exposes the entire Nextech API attached to a single global `Nextech` object. This version is useful if you don't want to use the modules directly with a standard AMD loader.
  - `minify` - Runs `combine`, plus [minifies](<http://en.wikipedia.org/wiki/Minification_(programming)>) Nextech.js using [UglifyJS2](https://github.com/mishoo/UglifyJS2) for a smaller deployable file.
  ...
  - `buildApps` - Builds the example applications (such as Nextech Viewer) to produce self-contained, minified, deployable versions in the `Build` directory.
  - `generateDocumentation` - Generates HTML documentation in `Build/Documentation` using [JSDoc 3](https://github.com/jsdoc3/jsdoc). More [details here](https://github.com/NextechGS/NextechJS/tree/master/Documentation/Contributors/DocumentationGuide).
...
- **Utility scripts** -- code coverage, static code analysis, and other utilities
  ...
  - `eslint-watch` - A never-ending task that watches your file system for changes to NextechJS and runs ESLint on any changed source files.
...
- **Testing scripts** -- build and run the unit tests
  ...
  - `test-release` - Runs all tests on the minified release version of built NextechJS.
- **Deployment scripts**
  - `deploy-s3` - Deploys the built NextechJS files, the npm package, and the zip file to Amazon S3. This requires having credentials set up for the S3 bucket to which you are deploying.
...
Travis triggers a build whenever someone opens a pull request or pushes code to the NextechJS repository. After the build has completed, at the bottom on the pull request, the status of the build is shown and you can access the build by clicking the "Details" link.

![Checks](checks_failed.jpg)

You can also access the build of any branch of NextechJS by going to the [NextechJS Branches](https://github.com/NextechGS/NextechJS/branches/all) page, and clicking the icon next to the branch name.

![Branches](branches.png)

Additional set up is required for deployment if you do not have commit access to NextechJS.
...
To configure Travis for deployment for a fork of NextechJS, you must have valid credentials to the S3 bucket.
...
To configure the additional commit statuses on GitHub for your fork of NextechJS, you need to generate a personal access token for Travis to use.
...
- Go to [travis-ci.org](https://travis-ci.org/) and select your fork of NextechJS
- Go to "More Options">"Settings"
